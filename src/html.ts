...
<body>
  ...
  <script>
    ...
    document.querySelectorAll('input[type="text"][name$="_image"]').forEach(input => {
      const wrapper = document.createElement('div')
      wrapper.style.cssText = 'margin-top: 0.5rem; padding: 1rem; border: 2px dashed var(--border); border-radius: 0.5rem; text-align: center; cursor: pointer; transition: all 0.2s;'
      wrapper.innerHTML = '<span>이미지를 드래그하거나 클릭하여 업로드</span>'
      ...
      wrapper.ondrop = async (e) => {
        ...
        if (file) await uploadImage(file, input, wrapper)
      }
      ...
      async function uploadImage(file, targetInput, statusEl) {
        statusEl.innerHTML = '<span>업로드 중...</span>'
        ...
        try {
          ...
          if (data.url) {
            ...
            statusEl.innerHTML = '<span style="color: green;">✓ 업로드 완료</span>'
            setTimeout(() => {
              statusEl.innerHTML = '<span>이미지를 드래그하거나 클릭하여 업로드</span>'
            }, 2000)
          } else {
            statusEl.innerHTML = '<span style="color: red;">업로드 실패</span>'
          }
        } catch (err) {
          statusEl.innerHTML = '<span style="color: red;">업로드 오류</span>'
        }
      }
      
      input.parentNode.insertBefore(wrapper, input.nextSibling)
      input.parentNode.insertBefore(fileInput, wrapper.nextSibling)
    })
  </script>
</body>
</html>
